...
const Chatbot = () => {
  ...
  const sendMessage = async () => {
    ...
    console.log(data.reply); // now this is your AI-generated answer
    ...
    {open && (
      <div className="fixed bottom-20 right-6 w-80 h-96 bg-slate-900 text-white rounded-xl shadow-xl flex flex-col z-50">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          {/* typing indicator */}
          {loading && (
            <p className="text-sm text-gray-400">{loadingMessages[Math.floor(Math.random() * loadingMessages.length)]}</p>
          )}
        </div>
